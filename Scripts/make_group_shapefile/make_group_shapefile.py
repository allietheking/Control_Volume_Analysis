...
if FR:
    ...
    polygon_shape_file = '../../Definitions/shapefiles/Agg_mod_contiguous_plus_subembayments_shoal_channel.shp'
...
# for AGG runs (includs now whole bay group added by allie)
    ...
    polygon_shape_file = '../../Definitions/shapefiles/Agg_mod_contiguous_141.shp'
...
for group in gdf_group.feature:
    ...
    ax.axis('off')
    if FR:
        plt.savefig('../../Definitions/plots_of_agg_groups/FR_group_%s.png' % group)
    else:
        plt.savefig('../../Definitions/plots_of_agg_groups/AGG_group_%s.png' % group)
    plt.close('all')

# now concatenate groups and connectivities and save in a shapefile
if FR:
    gdf_group.to_file('../../Definitions/shapefiles/group_definition_shapefile_FR.shp')
    gdf_connect.to_file('../../Definitions/shapefiles/group_connectivity_shapefile_FR.shp')
else:
    gdf_group.to_file('../../Definitions/shapefiles/group_definition_shapefile_AGG.shp')
    gdf_connect.to_file('../../Definitions/shapefiles/group_connectivity_shapefile_AGG.shp')